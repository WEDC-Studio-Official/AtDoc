...
위 규칙이 정의하는 것은 **Parser 동작**(내용을 파싱하지 않고 그대로 보존)입니다. 렌더링 측의 대응 의미론은 **인라인 코드**입니다 — Markdown의 백틱 `` `code` `` 과 같은 것입니다:

* Renderer는 `@raw`를 고정폭(monospace) 인라인 요소로 출력**해야 합니다(SHOULD)**. HTML Route는 `<code>`를 사용합니다.
* 이는 `@code`와 구별됩니다: `@code`는 **블록**(HTML에서는 `<pre><code>`)이고, `@raw`는 **인라인**입니다.
* `@kbd`와도 구별됩니다: `@kbd`는 물리적 키를 나타내며 관례적으로 테두리와 키캡 외형을 가집니다. `@raw`는 코드 텍스트이므로 고정폭 글꼴과 배경색만 있으면 됩니다.
* 이것이 Renderer가 내용을 파싱해도 된다는 뜻은 아닙니다 — 렌더링 의미론의 추가는 opaque domain의 파싱 규칙을 변경하지 않습니다.

> 왜 별도 노드가 아니라 `@raw`인가: `raw-escaped`는 언어 전체에서 유일하게 정규 이스케이프 메커니즘(`@]` / `@[` 와 대괄호 깊이 계산)을 갖춘 내용 모드이며, "내용이 파싱되어서는 안 된다"는 것이야말로 인라인 코드의 정의적 요건입니다. 두 사용 사례는 거의 완전히 겹칩니다 — 이 절 위의 모든 예시가 코드를 보여주고 있습니다.
