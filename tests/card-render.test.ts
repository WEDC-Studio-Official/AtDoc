...
// across both Adapters.ts routes — see the "Card Style v1" subsection of
// Block-Syntax-Specification.md §6 for the spec this exercises. Same rationale as color-render.test.ts: this
...
// palette, only hex + radius-N (see Block-Syntax-Specification.md §6).
{
  ...
  const inlineHtml = DocTranspiler.toInlineStyleHTML(node);
  check('named color token "blue" is not recognized by @card (no inline style)', inlineHtml === `<article>text</article>`, inlineHtml);
}
...
// @card(title){styles}[...] — title and styles slots are independent.
{
  ...
  const inlineHtml = DocTranspiler.toInlineStyleHTML(node);
  check('title renders alongside resolved styles', inlineHtml.includes('<header>API Key</header>') && inlineHtml.includes('background-color: #3366ff;') && inlineHtml.includes('border-radius: 12px;'), inlineHtml);
}
