...
export function tokenize(source: string): Token[] {
  ...
  while (i < n) {
    ...
    // string concat + function call for every byte of ordinary prose, which
    // dominated tokenize()'s allocation profile on large documents (see
    // docs/benchmark.md's Local Environment stage investigation).
    {
      const runStart = i;
      while (i < n) {
        const c = source[i];
