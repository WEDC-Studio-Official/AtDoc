...
| `28-styles-slot-on-unstyled-node.atd` | `@heading(1){radius-12}[...]`——把 `{styles}` 寫在文法沒有授予這個欄位的節點上 | **拋錯**：has no `{styles}` slot。Lexer 對任何節點名稱後面的 `{...}` 都會產生 STYLES token，所以這種寫法原本會被解析後靜默丟棄；只有 registry.ts 裡有 `styles` StyleSet 的節點才接受 |
